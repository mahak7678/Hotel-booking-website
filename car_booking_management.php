...
// Fetch car bookings
$carBookings = [];
$query = "SELECT id, name, pickup_location, drop_location, pickup_time, booking_time,total_amount FROM car_bookings ORDER BY booking_time DESC";
...
if (isset($_GET['delete'])) {
    $carBooking_id = $_GET['delete'];
    $delete_query = "DELETE FROM car_bookings WHERE id = $carBooking_id";
    if (mysqli_query($con, $delete_query)) {
        echo "<script>alert('Booking deleted successfully'); window.location.href='car_booking_management.php';</script>";
    } else {
...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $query .= " WHERE name LIKE '%$search%' OR pickup_location LIKE '%$search%' OR drop_location LIKE '%$search%'";
    $result = mysqli_query($con, $query);
    $carBookings = [];
    while ($row = mysqli_fetch_assoc($result)) {
        $carBookings[] = $row;
    } // <-- This closes the second while loop
}
?>
...
<head>
  ...
  <title>Car Booking Management - Admin Panel</title>
...
<header class="bg-primary text-white sticky-top shadow">
  <nav class="navbar navbar-expand-lg navbar-dark">
    <div class="container-fluid">
      <a class="navbar-brand" href="#"><i class="fas fa-car me-2"></i> Car Booking Management</a>
    </div>
  </nav>
</header>
  ...
  if (confirm('Are you sure you want to logout?')) {
    ...
    <div class="col-md-9 main-content">
      <div class="dashboard-header">
        Welcome to Car Booking Management, <?= htmlspecialchars($_SESSION["admin"]) ?>
      </div>

      <form class="mb-4" method="get" action="car_booking_management.php">
        <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" class="form-control" placeholder="Search by name, pickup or drop location" />
      ...
      <div class="row mb-4">
        <div class="col-md-12 mb-4">
          <div class="card shadow">
            <div class="card-header bg-info text-white">Car Booking List</div>
            <div class="card-body">
              <?php if (count($carBookings) > 0): ?>
                  ...
                  <tbody>
                    <?php foreach ($carBookings as $booking): ?>
                      <tr>
                        ...
                        <td><?= date('Y-m-d H:i', strtotime($booking['pickup_time'])) ?></td>
                        <td><?= date('Y-m-d H:i:s', strtotime($booking['booking_time'])) ?></td>
						  <td><?= htmlspecialchars($booking['total_amount']) ?></td>
                        <td>
                          <a href="?delete=<?= $booking['id'] ?>" class="btn-delete btn btn-sm">Delete</a>
                        </td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
